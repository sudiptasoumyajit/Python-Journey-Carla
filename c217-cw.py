...
try:
    ...
    def _Radar_callback(radar_data):
        ...
        radar_current_rotation=radar_data.transform.location
        ...
        for detect in radar_data:
            ...
            carla.Transform(carla.Location(),carla.Rotation(pitch=new_pitch,yaw=new_yaw,roll=new_roll)).transform(forward_view)
            #write the code for draw points
            def clamp(min_v,max_v,value):
            	return(min_v,min(value,max_v))
    norm_velocity=detect.velocity/radar_velocity_range
    r=int(clamp(0.0,1.0,1.0-norm_velocity)*225.0)
    g=int(clamp(0.0,1.0,1.0-abs(norm_velocity))*255.0)
    debug.draw_point(radar_data.transform.location+forward_view,size=0.075,life_time=0.06,persistent_lines=False,color=carla.Color(r,g,b))
